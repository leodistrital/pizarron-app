	...
	if(formPersona.length){
		formPersona.validate({
			...
			submitHandler:function(form){
				...
				var paso = 0; checkbox = $('#contEvent input[type=checkbox]');
				if(checkbox.length) {
	...
	if(formConyuge.length){
		...
		if(deleteReg.length){
			deleteReg.on('click',function(){
				...
				createAlertElim(formConyuge,'Confirmación','<p>Este registro se eliminará del sistema. <br>¿Esta seguro de eliminar el registro?</p>',borra_conyogue);
			});
		}
	...
	if(formConyuge.length){
		formConyuge.validate({
			...
			submitHandler:function(form){
				...
				$.ajax({
					type:"POST",
					url:"ajax/libajax.php?funcion=asistente&&operacion=crear_persona",
					data:new FormData(form),
					cache: false,
					contentType: false,
					processData: false,
		...
		if(deleteReg.length){
			deleteReg.on('click',function(){
				createAlertElim(formConyuge,'Confirmación','<p>----Este registro se eliminará del sistema. <br>¿Esta seguro de eliminar el registro?</p>',borra_asistente);
...
function createItem(btn){
	...
	if(html.find('input')){
		...
		if(input.hasClass('checkDireccion')){input.prop('checked', false)}
		input.each(function(){
...
function asistentes() {
	...
	$.ajax({
			...
			data:  {"codigo":  $('#ultimo_id').val()}
		}).done(function(data){	crear_div_asis(data,id_pade); });	
	});
}
...
function crear_div_asis(id_asistente, id_pade) {  
	var nuevo_div="<p class ='model-asis' id='div_sis_"+ id_asistente.trim() +"' ><label>&nbsp;</label><a href='crear_asistente.php?codigo_maestro="+id_pade+"&&id_asistente="+id_asistente+"' class='btnExtra abrir_fan_asis' >Información Asistente</a><span class='remove' data-codigo='"+id_asistente+"' onclick='borrado_sistente("+id_asistente+")'>x</span></p> ";
...
if (r == true) {
    $.ajax({
		type: "POST",
		url:"ajax/libajax.php?funcion=asistente&&operacion=borrar&&id_asistente="+id_asistente,
		data:  {"codigo":  $('#ultimo_id').val()}
	});
