...
function editar_conyugue_leo(){
	//printArray($_POST);
	 $campos="nom_per='".correcion_html_utf8($_REQUEST["nom-c"])."', ape_per='".correcion_html_utf8($_REQUEST["ape-c"])."' ,  mai_per='".$_REQUEST["mail-c"]."', tof_per='".$_REQUEST["tel-c"]."', obs_per='".$_REQUEST["obs-c"]."',   coy_per='".$_REQUEST["ultimo_id_padre"]."' , temp_per=0  , est_coy_per=1  ";
...
function editar_empresa_leo (){
	...
	$arreglo_empresas = split("@@",$_REQUEST['arreglo_empesa_caja']);
	for($i=0;$i<count($arreglo_empresas)-1;$i++){
		$arreglo_datos = explode("||",$arreglo_empresas[$i]);
		//print_r($arreglo_datos);
		$campos=" cod_per_dpe , cod_emp, cod_suc, dep_dep,  	car_dpe, mail_dpe , tel_dpe , dir_dpe ";
		$valores=" '".$codigo_maestro."', '".$arreglo_datos[0]."', '".$arreglo_datos[1]."', '".correcion_html_utf8($arreglo_datos[2])."','".correcion_html_utf8($arreglo_datos[3])."','".correcion_html_utf8($arreglo_datos[4])."' ,'".correcion_html_utf8($arreglo_datos[5])."'  ,'".correcion_html_utf8($arreglo_datos[6])."'   " ;
		//echo "<br>";
