        ...
        formOlvClave.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=perfil&&operacion=recuperar_perfil",
                    ...
                    var mensaje =
                        "<p>La información se ha almacenado exitosamente en nuestra base de datos. <br><br>Presione aceptar para continuar.</p>";
                    createAlert(
                        formOlvClave,
                        "Registro exitoso",
                        mensaje,
...
function validateFormPersona() {
    ...
    if (formPersona.length) {
        formPersona.validate({
            ...
            submitHandler: function (form) {
                var paso = 0;
                checkbox = $("#contEvent input[type=checkbox]");
                if (checkbox.length) {
                    var estadoCivil = $("#civ-per");
                    checkbox.each(function () {
                        if ($(this).prop("checked") && estadoCivil.val() != 1) {
                            alert("Verifique el estado civil");
                            paso = 1;
                            estadoCivil.focus();
                        }
                    });
                ...
                if (paso == 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        url:
                            "ajax/libajax.php?funcion=persona&&operacion=crear_persona",
                        data: new FormData(form),
                        cache: false,
                        contentType: false,
                        processData: false,
						...
						*/
                        var mensaje =
                            "<p>Confirmar dirección de correspondencia</p>";
                        createAlertConfirma(
                            formPersona,
                            "Confirmar",
                            mensaje,
        ...
        $(".abrir_fan_asis").on("click", function (e) {
            e.preventDefault();
        ...
        $("#toggleFC-as").on("click", function (e) {
            e.preventDefault();
            openFormConyuge($(this).attr("href"));
        });
        ...
        loadCheckDireccion(formPersona);
    }
}
        ...
        formConyuge.validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=conyugue&&operacion=crear_persona",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
...
function borra_conyogue() {
    var codigo_cony = $("input[name='ultimo_id_conyugue']");
    console.log(codigo_cony.val());
    $.ajax({
        type: "POST",
        url: "ajax/libajax.php?funcion=conyugue&&operacion=borrar",
        data: { conyugue: codigo_cony.val() },
        cache: false,
        //contentType: false,
        //processData: false,
...
function validateFormAsistente() {
    var formConyuge = $("#formasistente");
    if (formConyuge.length) {
        formConyuge.validate({
            rules: {
                "nom-c": "required",
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=asistente&&operacion=crear_persona",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        $("#contFC .btnClose").on("click", function (e) {
            e.preventDefault();
            closeFormConyuge();
            //borra_asistente('closeFormConyuge');
        });
        //textarea length
        loadMaxlength($(".maxLength"), 128);
    }
        ...
        formEmpresa.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=empresa&&operacion=crear_empresa",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        formSucursal.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=surcursal&&operacion=crear_empresa_sucursal",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        formEvento.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=evento&&operacion=crear_evento",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
...
function validateFormReporte() {
    ...
    if (formBusqReporte.length) {
                ...
                if ($("#prev").val() != 1) {
                    ...
                    $.ajax({
                        ...
                        createAlert(
                            formBusqReporte,
                            "Reporte creado",
                            "<p>El reporte se ha creado exitosamente y se ha descargado a su disco duro. <br><br>Importe el archivo a excel para visualizarlo y modificarlo.</p>"
                        );
                        ...
                        if (rep == 2) {
                            window.open("informe_imp.php");
                        }
                        if (rep == 3) {
                            window.open("informe_asistente.php");
                        }
                    });
                }
            },
        ...
        formBusqReporte.find("#genExcelS").on("click", function () {
            $("#prev").val("1");
            rep = 2;
            $("#formBusqReporte").submit();
        });

        formBusqReporte.find("#genExcelSec").on("click", function () {
            $("#prev").val("1");
            rep = 3;
            $("#formBusqReporte").submit();
        });
...
function createItem(btn) {
    var item = btn.parents(".contRemove"),
        content = btn.parents("ul");
    ...
    if (html.find(".btnMas")) {
        html.find(".btnMas").remove();
        html.append('<span class="remove">x</span>');
    }
    ...
    if (html.find("select")) {
        ...
        select.each(function () {
            if (html.find("label[for=" + $(this).attr("id") + "]")) {
                html.find("label[for=" + $(this).attr("id") + "]").remove();
            }
            var ultimo = content.find("li:last-child select"),
                cont = 1;
            if (ultimo.data("num")) {
                cont = ultimo.data("num");
            }
            $(this)
                .attr("id", $(this).attr("id") + "-" + (cont + 1))
                .attr("name", $(this).attr("name") + "-" + (cont + 1))
                .data("num", cont + 1);
    ...
    if (html.find("input")) {
        ...
        if (input.hasClass("checkDireccion")) {
            input.prop("checked", false);
        }
        input.each(function () {
            var ultimo = content.find("li:last-child input"),
                cont = 1;
            if (ultimo.data("num")) {
                cont = ultimo.data("num");
            }
        ...
        buttons: {
            "Si, eliminar registro": function () {
                ...
                createAlert(
                    $("body"),
                    "Confirmación",
                    "<p>El registro se ha eliminado exitosamente!</p>"
                );
            },
        ...
        formCorresp.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=correspondencia&&operacion=editar_correspondecia",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
...
function buscarDatos(selectSuc, resutados) {
    var Telefono = selectSuc
        .parents(".contRemove")
        .find("input[id^='tel-emp']");
    var Direccion = selectSuc
        .parents(".contRemove")
        .find("input[id^='dir-emp']");
        ...
        formSegmento.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=segmento&&operacion=crear_segmento",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        formSector.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=sector&&operacion=crear_sector",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        formTitulo.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=titulo&&operacion=crear_titulo",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        formPerfil.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=perfil&&operacion=editar_perfil",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
...
function validar_checks() {
    ...
    if (
        $("#12_tit").is(":checked") == false &&
        $("#1_nom").is(":checked") == false &&
        $("#2_ape").is(":checked") == false &&
        $("#3_email").is(":checked") == false &&
        $("#4_cel").is(":checked") == false &&
        $("#5_gen").is(":checked") == false &&
        $("#6_prof").is(":checked") == false &&
        $("#7_estc").is(":checked") == false &&
        $("#8_nomc").is(":checked") == false &&
        $("#9_ciur").is(":checked") == false
    ) {
...
function loadCheckDireccion(parent) {
    var check = parent.find(".checkDireccion");
    if (check.length) {
        check.on("click", function (e) {
            cambiarCheckDireccion($(this));
        });
    }
}

function cambiarCheckDireccion(obj) {
    var allObj = $(".checkDireccion:checked");
    if (obj.prop("checked")) {
...
function asistentes() {
    var btn_newasis = $("#btn-newasis");
    //console.log(btn_newasis)
    var id_pade = $("#ultimo_id").val();
    btn_newasis.on("click", function (e) {
        $.ajax({
            type: "POST",
            url: "ajax/libajax.php?funcion=asistente&&operacion=crear",
            data: { codigo: $("#ultimo_id").val() },
        }).done(function (data) {
            crear_div_asis(data, id_pade);
        });
    });
}

function crear_div_asis(id_asistente, id_pade) {
    var nuevo_div =
        "<p class ='model-asis' id='div_sis_" +
        id_asistente.trim() +
        "' ><label>&nbsp;</label><a href='crear_asistente.php?codigo_maestro=" +
        id_pade +
        "&&id_asistente=" +
        id_asistente +
        "' class='btnExtra abrir_fan_asis' >Información Asistente</a><span class='remove' data-codigo='" +
        id_asistente +
        "' onclick='borrado_sistente(" +
        id_asistente +
        ")'>x</span></p> ";
    $(".div-asis").append(nuevo_div);
    $(".abrir_fan_asis").on("click", function (e) {
        e.preventDefault();
        openFormConyuge($(this).attr("href"));
    });
...
function borrado_sistente(id_asistente) {
    var r = confirm("Desea borrar asistente ?");
    if (r == true) {
        $.ajax({
            type: "POST",
            url:
                "ajax/libajax.php?funcion=asistente&&operacion=borrar&&id_asistente=" +
                id_asistente,
            data: { codigo: $("#ultimo_id").val() },
        });
        var id_borrar = "#div_sis_" + id_asistente;
        $(id_borrar).remove();
    }
}

// funcion busqueda sector
function formBusqEnvios() {
    var divResult = $("#divResult");
    gLoading("open");
    fetch("result.php?tipo=60")
...
//Formulario nuevo evento
function validateFormEnvio() {
    var formEvento = $("#formEnvio");
    // debugger;
    ...
    if (formEvento.length) {
        formEvento.validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST",
                    url:
                        "ajax/libajax.php?funcion=envio&&operacion=crear_envio",
                    data: new FormData(form),
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        //Iniciar proceso
        var deleteReg = formEvento.find(".inienvioReg");
        if (deleteReg.length) {
            deleteReg.on("click", function () {
                createAlertPro(
                    formEvento,
                    "Confirmación",
                    "<p>¿Esta seguro de iniciar este proceso?</p>",
                    envioProceso
                );
            });
        }
...
function borrar_envio() {
    $.ajax({
        type: "POST",
        url: "ajax/libajax.php?funcion=evento&&operacion=borrar_envio",
        data: { edi: $("#edi").val() },
    }).done(function () {
        closeFancy();
    });
...
function envioProceso() {

    var id = document.querySelector('#edi').value;
    console.log("arrancamos", id);
    // return false;
    var url = "envio/whatsapp.php";
    fetch(url, {
        method: "POST",
        headers: {
...
function createAlertPro(form, titulo, contenido, funcionExt) {
    form.append('<div id="gAlert">' + contenido + "</div>");
    var gAlert = $("#gAlert");
    gAlert.dialog({
        title: titulo,
        ...
        buttons: {
            "Si, iniciar proceso": function () {
                ...
                createAlert(
                    $("body"),
                    "Confirmación",
                    "<p>El proceso inicío exitosamente!</p>"
                );
            },
            "No, cancelar esta acción": function () {
                $(this).dialog("close").dialog("destroy");
